...
- 5 Issues I found:
- The app allows the user to enter duplciate songs which do not make sense since each song is unique so having multiple of the same song is redundant.
- The hype ratio never changes no matter how many songs that are hype or are not hype we add.
- Even when changing the genre filter changing the top 3 songs always stay the same.
- When changing the minimum energy the filtered songs did not change much
- No error messages pop up when entering invalid inputs.
- When using lucky pick mixed songs are never chosen. 
